"""
Optuna hyper‑parameter tuning (GPU‑max‑util) – CatBoost top‑3 Formula 1 prediction.

* draait volledig op de GPU (`task_type='GPU'`, `devices='0'`)
* gebruikt Logloss‑metric om CPU‑fallback te vermijden
* optimaliseert mean F1 − 0.5·σ(F1) en threshold (0.40‑0.55)
...
def objective(trial):
    params = {
        ...
        "learning_rate": trial.suggest_float("lr", 0.01, 0.045, log=True),
        ...
        "metric_period": 100,
        "max_gpu_memory": 0.9 if args.gpu else None,
